...
class File extends events.EventEmitter {
    ...
    this.api.request(req, (err, response) => {
      ...
      if (this.directory) {
        ...
        const folder = nodes.find(
          (node) =>
            node.k && // the root folder is the one which "n" equals the first part of "k"
  ...
  download(options, cb) {
    ...
    if (this.directory)
      throw Error("Can't download: folder download isn't supported"); // If options.returnCiphertext is true then the ciphertext is returned.
    ...
    cs.request(req, (err, response) => {
      ...
      if (maxConnections === 1) {
        ...
        const getChunk = function getChunk() {
          ...
          const r = requestModule(
            response.g + '/' + currentOffset + '-' + currentMax,
          );
          r.on('end', getChunk);
          r.on('error', handleConnectionErrors);
          r.on('response', handleMegaErrors);
  ...
  link(options, cb) {
    ...
    let url$$1 = `https://mega.nz/${this.directory ? 'folder' : 'file'}/${
      this.downloadId
    }`;
  ...
  static fromURL(opt) {
    ...
    if (url$$1.path.match(/\/(file|folder)\//) !== null) {
      ...
      const fileHandler = url$$1.path.substring(
        url$$1.path.lastIndexOf('/') + 1,
        url$$1.path.length + 1,
      );
...
class MutableFile extends File {
  ...
  mkdir(opt, cb) {
    ...
    if (shares.length > 0) {
      request$$1.cr = makeCryptoRequest(
        this.storage,
        [
          {
            nodeId: 'xxxxxxxx',
  ...
  importFile(sharedFile, cb) {
    ...
    if (!sharedFile.key)
      return cb(Error("Can't import files without encryption keys")); // We need file attributes
